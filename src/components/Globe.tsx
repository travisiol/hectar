 ...
 *
 * A plot with no market is an outline; one that trades burns signal-bright in
 * proportion to its activity. Signal appears nowhere else, so on a sphere of
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (const star of STARS) {
        ...
        ctx.fillStyle = `rgba(226, 242, 233, ${star.a})`;
        ctx.fill();
      }
      ...
      halo.addColorStop(0, "rgba(46, 226, 123, 0.24)");
      halo.addColorStop(0.45, "rgba(46, 226, 123, 0.09)");
      halo.addColorStop(1, "rgba(46, 226, 123, 0)");
      ctx.beginPath();
      ...
      ocean.addColorStop(0, "#14261d");
      ocean.addColorStop(0.55, "#0b1a13");
      ocean.addColorStop(1, "#020806");
      ctx.beginPath();
      ...
      limb.addColorStop(0, "rgba(2, 6, 4, 0)");
      limb.addColorStop(1, "rgba(2, 6, 4, 0.78)");
      ctx.beginPath();
      ctx.arc(f.cx, f.cy, radius, 0, Math.PI * 2);
      ctx.fillStyle = limb;
      ctx.fill();

      ctx.strokeStyle = "rgba(210, 232, 219, 0.5)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.arc(f.cx, f.cy, radius, 0, Math.PI * 2);
      ctx.stroke();
      ...
      ctx.lineWidth = 0.6;
      ctx.strokeStyle = "rgba(200, 216, 207, 0.10)";
      for (const line of graticule) strokePolyline(line, f, false);

      ctx.lineWidth = 0.8;
      ctx.strokeStyle = "rgba(214, 236, 223, 0.44)";
      ...
      for (let b = 0; b < bands.length; b += 1) {
        ...
        ctx.fillStyle = `rgba(206, 226, 214, ${band.fill})`;
        ctx.fill();
        ctx.strokeStyle = `rgba(206, 226, 214, ${band.stroke})`;
        ctx.lineWidth = 0.7;
        ctx.stroke();
      }
      ...
      for (let i = 0; i < parcels.length; i += 1) {
        ...
        // Cyan once a plot has the most owners: at a glance, green is a
        // market that exists and cyan is one people are piling into.
        const alpha = (0.35 + heat * 0.65) * Math.min(1, c.z * 2.2);
        const rgb =
          market.owners >= CROWDED_OWNERS ? "92, 228, 255" : "46, 226, 123";
      ...
      emphasise(selectedRef.current, "#2ee27b", 2.2);
