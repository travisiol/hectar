export const siteConfig = {
  ...
  url: process.env.NEXT_PUBLIC_SITE_URL ?? "https://hectar.example",
  x: process.env.NEXT_PUBLIC_HECTAR_X ?? null,
  discord: process.env.NEXT_PUBLIC_HECTAR_DISCORD ?? null,
} as const;
...
export const world = {
   ...
   * NEXT_PUBLIC_HECTAR_CONTRACT_ADDRESS is set — from then on the map is
   * drawn from the chain's own claim bitmap.
   */
  placeholderClaims: envInt(process.env.NEXT_PUBLIC_HECTAR_PLACEHOLDER_CLAIMS, 3),
  maxPerWallet: envInt(process.env.NEXT_PUBLIC_HECTAR_MAX_PER_WALLET, 5),
} as const;
...
export const claimConfig = {
  contractAddress: envOrNull(
    process.env.NEXT_PUBLIC_HECTAR_CONTRACT_ADDRESS,
  ) as `0x${string}` | null,
  /** Price per parcel in ETH as a decimal string, e.g. "0.04". */
  priceEth: envOrNull(process.env.NEXT_PUBLIC_HECTAR_PRICE_ETH),
  isLive: process.env.NEXT_PUBLIC_HECTAR_LIVE === "true",
} as const;
